...
#define fd_stem_publish( stem, out_idx, sig, chunk, sz, ctl, tsorig, tspub ) \
  do { (void)(stem); (void)(out_idx); (void)(sig); (void)(chunk); (void)(sz); \
       (void)(ctl); (void)(tsorig); (void)(tspub); fake_publish_cnt++; } while(0)

static int   fake_shred_len = -1;
static uchar fake_shred[ FD_SHRED_MAX_SZ ];

static int
fake_shredb_query( ulong   slot,
                   uint    shred_idx,
                   uchar * out ) {
  (void)slot; (void)shred_idx;
  if( fake_shred_len<0 ) return -1;
  memcpy( out, fake_shred, (ulong)fake_shred_len );
  return fake_shred_len;
}

#define fd_shredb_query( store, slot, shred_idx, out )         fake_shredb_query( (slot), (shred_idx), (out) )
#define fd_shredb_query_highest( store, slot, shred_idx, out ) fake_shredb_query( (slot), (shred_idx), (out) )

static void
fake_keyguard_sign( uchar *       signature,
                    uchar const * data,
                    ulong         data_len ) {
  memset( signature, 0x61, 64UL );
  if( FD_LIKELY( data_len ) ) signature[ 0 ] = data[ 0 ];
}

#define fd_keyguard_client_sign( client, signature, data, data_len, type ) \
  fake_keyguard_sign( (signature), (data), (data_len) )
...
/* Same input helpers as fuzz_httpserver.c: bytes come straight from the
   fuzz input and wrap around when it runs out. */
...
static void
unstructured_take( struct Unstructured * u,
                   ulong                 len,
                   void *                out ) {
  uchar * o = (uchar *)out;
...
static uchar
rand_uchar( struct Unstructured * u ) {
  uchar v; unstructured_take( u, sizeof(v), &v ); return v;
}

static uint
rand_uint( struct Unstructured * u ) {
  uint v; unstructured_take( u, sizeof(v), &v ); return v;
}

static ulong
rand_ulong( struct Unstructured * u ) {
  ulong v; unstructured_take( u, sizeof(v), &v ); return v;
}

static void
rand_bytes( struct Unstructured * u,
            ulong                 len,
            uchar *               p ) {
...
static ulong
rand_range( struct Unstructured * u,
            ulong                 n ) {
...
static void
pick_addr( struct Unstructured * u,
           uint *           ip4,
           ushort *         port ) {
  /* One time in eight a random address, else one of the fixed ones. */
  if( FD_LIKELY( rand_uchar( u ) % 8U ) ) {
    ulong idx = rand_range( u, ADDR_CNT );
    *ip4  = addr_ip4 [ idx ];
    *port = addr_port[ idx ];
  } else {
    *ip4  = rand_uint( u );
    *port = (ushort)rand_uint( u );
  }
}

static void
dispatch( struct Unstructured * u,
          uchar const *    pkt,
          ulong            pkt_sz,
          uint             saddr,
          ushort           sport ) {
...
static void
action_pong( struct Unstructured * u ) {
  uchar pkt[ PKT_SZ_PONG ];
  memset( pkt, 0, sizeof(pkt) );

  ulong  peer = rand_range( u, PEER_CNT );
  uint   saddr;
  ushort sport;
  ...
  if( FD_LIKELY( rand_uchar( u ) % 16U ) ) memcpy( pkt+PONG_OFF_FROM, peer_pub[ peer ], 32UL );
  else                                     memcpy( pkt+PONG_OFF_FROM, ctx->identity_public_key.uc, 32UL );
  ...
  if( FD_LIKELY( rand_uchar( u ) % 8U ) ) {
    ...
    rand_bytes( u, sizeof(pong_hash), pong_hash );
  }
  memcpy( pkt+PONG_OFF_HASH, pong_hash, 32UL );

  uchar sig[ 64 ];
  if( FD_LIKELY( rand_uchar( u ) & 7U ) ) {
    fd_ed25519_sign( sig, pkt+PONG_OFF_HASH, 32UL, peer_pub[ peer ], peer_priv[ peer ], harness_sha );
  } else {
    rand_bytes( u, sizeof(sig), sig );
  }
  memcpy( pkt+PONG_OFF_SIG, sig, 64UL );

  if( FD_UNLIKELY( !(rand_uchar( u ) & 15U) ) ) {
    ulong off = rand_ulong( u ) % sizeof(pkt);
    pkt[ off ] = rand_uchar( u );
  }
...
static void
action_request( struct Unstructured * u ) {
  ...
  int odd_tag  = (rand_uchar( u ) % 8U==0U);
  int odd_size = (rand_uchar( u ) % 8U==0U);

  uint tag = odd_tag ? ODD_TAGS[ rand_range( u, sizeof(ODD_TAGS)/sizeof(ODD_TAGS[0]) ) ]
                     : TAGS    [ rand_range( u, sizeof(TAGS)    /sizeof(TAGS[0])     ) ];

  ulong pkt_sz = tag==FD_REPAIR_KIND_ORPHAN ? PKT_SZ_ORPHAN : PKT_SZ_SHRED;
  ulong sign_sz = tag==FD_REPAIR_KIND_ORPHAN ? 88UL : 96UL;

  if( FD_UNLIKELY( odd_size ) ) pkt_sz = rand_range( u, PKT_SZ_SHRED + 8UL );
  ...
  if( FD_LIKELY( have_last && (rand_uchar( u ) & 3U) ) ) {
    peer  = last_peer;
    saddr = last_saddr;
    sport = last_sport;
  } else {
    pick_addr( u, &saddr, &sport );
    peer = rand_range( u, PEER_CNT );
  }

  if( FD_LIKELY( pkt_sz>=sizeof(uint) ) ) memcpy( pkt, &tag, sizeof(uint) );

  if( FD_LIKELY( pkt_sz>=PKT_SZ_ORPHAN ) ) {
    if( FD_LIKELY( rand_uchar( u ) % 8U ) ) memcpy( pkt+PKT_OFF_FROM, peer_pub[ peer ], 32UL );
    else                                    memcpy( pkt+PKT_OFF_FROM, ctx->identity_public_key.uc, 32UL );

    if( FD_LIKELY( rand_uchar( u ) % 8U ) ) memcpy( pkt+PKT_OFF_TO, ctx->identity_public_key.uc, 32UL );
    else                                    rand_bytes( u, 32UL, pkt+PKT_OFF_TO );

    long now_ms = FD_NANOSEC_TO_MILLI( fd_log_wallclock() );
    ulong ts;
    if( FD_LIKELY( rand_uchar( u ) % 8U ) ) ts = (ulong)(now_ms - (long)rand_range( u, 256UL ));
    else                                    ts = rand_ulong( u );
    memcpy( pkt+PKT_OFF_TS, &ts, sizeof(ulong) );

    uint nonce = rand_uint( u );
    memcpy( pkt+PKT_OFF_NONCE, &nonce, sizeof(uint) );

    ulong slot = rand_ulong( u ) % 1024UL;
    memcpy( pkt+PKT_OFF_SLOT, &slot, sizeof(ulong) );
  }

  if( FD_LIKELY( pkt_sz>=PKT_SZ_SHRED ) ) {
    ulong shred_idx;
    if( FD_LIKELY( rand_uchar( u ) & 7U ) ) shred_idx = rand_ulong( u ) & fd_ulong_mask( 0, 14 );
    else                                shred_idx = rand_ulong( u );
  ...
  // at 25 % the shred store has nothing for this request.
  if( FD_UNLIKELY( rand_uchar( u ) % 4U==0U ) ) {
    fake_shred_len = -1;
  } else {
    fake_shred_len = (int)( FD_SHRED_MIN_SZ + rand_range( u, FD_SHRED_MAX_SZ-FD_SHRED_MIN_SZ+1UL ) );
    rand_bytes( u, (ulong)fake_shred_len, fake_shred );
    ushort parent_off;
    if( FD_LIKELY( rand_uchar( u ) & 3U ) ) parent_off = (ushort)rand_range( u, 4UL );
    else                                parent_off = (ushort)rand_uint( u );
    memcpy( fake_shred+offsetof( fd_shred_t, data.parent_off ), &parent_off, sizeof(ushort) );
  }

  if( FD_UNLIKELY( !(rand_uchar( u ) & 15U) ) && pkt_sz ) {
    ulong off = rand_ulong( u ) % pkt_sz;
    pkt[ off ] = rand_uchar( u );
  }
...
static void
action_raw( struct Unstructured * u ) {
  uchar pkt[ 256 ];
  ulong pkt_sz = rand_range( u, sizeof(pkt)+1UL );
  rand_bytes( u, pkt_sz, pkt );

  if( FD_LIKELY( pkt_sz>=sizeof(uint) ) && (rand_uchar( u ) & 1U) ) {
    ...
    uint tag = TAGS[ rand_range( u, 6UL ) ];
...
LLVMFuzzerTestOneInput( uchar const * data,
  ...
  struct Unstructured u = { .data = data, .size = size, .used = 0UL };
  ...
  ulong n_actions = 1UL + rand_range( &u, 12UL );
  for( ulong i=0UL; i<n_actions; i++ ) {
    switch( rand_uchar( &u ) % ACT_CNT ) {
      case ACT_PONG:    action_pong   ( &u ); break;
      case ACT_REQUEST: action_request( &u ); break;
      case ACT_RAW:     action_raw    ( &u ); break;
      case ACT_ROTATE:  fd_rserve_maybe_rotate( ctx->rserve, rand_ulong( &u ) ); break;
    }
  }
